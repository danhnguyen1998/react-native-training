...
const fetchPost = async (url, body, messErr = null, isAuthorization = true) => {
  try {
    ...
    if (isAuthorization) headers.Authorization = await AsyncStorage.getItem("token");
...
const fetchPostFormData = async (url, formData, messErr = null, isAuthorization = true, method = "POST") => {
  try {
    let headers = {
    ...
    if (response.status === 500) throw Error(result.message ? result.message : messErr);
...
const fetchGet = async (url, params = null, messErr = null, isAuthorization = true) => {
  try {
    let headers = {
      Accept: "application/json",
      "Content-Type": "application/json"
    ...
    const response = await fetch(`${API_URL}/api/${url}${queryString}`, requestInit);
    if (response.status === 403) throw Error("invalidToken");
    if (response.status === 200) return await response.json();
    if (messErr) throw Error(messErr);
  } catch (error) {
